...
> + 앱 흐름 `e2e/flows/<서비스>.mjs`. 회차 기록은 `docs/qa/runs/`. 명세: `docs/spec/mcp-server.md`.
...
## Scenario S8 — 앱 구동 흐름 (e2e/flows/mcp.mjs — 접근성 쿼리 금지, CSS/text 로케이터만)
...
- **CASE-mcp-075** e2e: AI 화면에 "접속 키를 바꾼 뒤" 재등록 안내 + Claude/Codex 재등록 복사 버튼이 보이고, 재발급 후 재등록 명령이 새 키를 담는다. (agents.token AC-3) → `e2e/flows/mcp.mjs` [자동]
