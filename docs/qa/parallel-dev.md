...
## Scenario S1 — e2e 안전 가드가 쪼갠 흐름 파일까지 덮는다 (불변식 1)
→ `e2e/isolation.test.ts` [자동]

분할 전 이 가드는 `smoke.mjs` **한 파일만** 읽었다. 흐름을 `e2e/flows/*.mjs` 로 빼면
가드가 빈 껍데기를 검사하게 된다 — 과거 실 사용자 DB 를 파괴한 사고의 재발 통로다.

- **CASE-pdev-001** 흐름 파일에 실 앱 userData 경로(`Application Support`)가 있으면 `npm test` 실패. `smoke.mjs` 뿐 아니라 `e2e/flows/*.mjs` 전부가 대상.
- **CASE-pdev-002** 흐름 파일의 `rmSync(...)` 는 임시 userData(`USER_DATA`) 대상일 때만 통과. 그 외 경로면 실패.
- **CASE-pdev-003** 검사 대상 파일 목록이 **하드코딩이 아니다** — `e2e/flows/` 에 새 파일을 놓으면 등록 없이 자동으로 검사에 들어온다. (하드코딩이면 새 서비스 흐름이 가드를 우회한다.)
