...
> 회차 기록은 `docs/qa/runs/`. 규칙 정본: `docs/agents/parallel-dev.md`.
>
> **이 계획의 목적은 기능 검증이 아니라 "약화 안 됨" 증명이다.** 공용 파일 6곳을 서비스별로
> 쪼개면서 프로젝트의 가드(`docs/agents/guardrails.md` 1·3·5)를 강제하던 기계 장치들이
...
- **CASE-pdev-020** 서비스별 마이그레이션을 모두 적용하면 기존 테이블 **17개가 전수 생성**된다(분할 전 목록을 손으로 적어 둔 기준과 이름 대조). 이 대조가 "서비스 모듈 하나가 등록부에서 통째로 빠진" 경우를 잡는 유일한 그물이다 — 선언과 생성이 함께 줄면 다른 검사는 통과해 버리기 때문이다. **줄었는지만 본다(포함 검사)** — 서비스가 자기 테이블을 더하는 건 정상이라, 정확히 일치로 보면 테이블을 더할 때마다 이 공용 테스트가 깨져 병렬 개발 규칙과 충돌한다.
...
→ `npm run e2e -- --all` [수동 — docker test-db 필요. 이 시나리오는 라벨 집합 대조라 전체가 필요하다]

- **CASE-pdev-050** 분할 후 `npm run e2e -- --all` 의 PASS 라벨 집합이 분할 전과 **동일**하다(개수만이 아니라 라벨 대조 — 조용히 사라진 검사 0).
...
## 범위 밖
- 각 서비스의 기능 동작 — 해당 서비스 TestPlan(`db-design`·`db-remote`·`db-connections`·`ai-server`)의 몫.
- 병합 충돌이 "실제로" 줄었는지 — 여러 브랜치를 실제로 굴려야 나오는 결과라 자동 검증 대상이 아니다. 분할이 구조적으로 충돌 지점을 없앴는지는 파일 소유권 표(`docs/agents/parallel-dev.md`)로 확인한다.
