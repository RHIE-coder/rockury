...
- **e2e 스위트 번호.** API 스위트를 13~16 으로 잡았는데, 기존 `12-cold-restart.mjs` 가
  그보다 앞서 돈다. 즉 **API 가 만든 상태는 재시작 검증에 안 들어간다.** 고치려면
  cold-restart 를 뒤로 옮겨야 하는데 그건 공용 파일 순서 변경이라 여기서 하지 않았다.
