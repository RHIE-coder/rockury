...
- **CASE-apistudio-096 (관측이 아니다)** 가짜 응답은 **실행 기록으로 안 남는다** — 남으면 판정이 "선언 대 선언"이 되어 늘 이상 없음이 된다. 응답에 가짜임을 밝히는 헤더가 붙는다. 켜면 로컬 주소가 뜨고 끄면 그 주소가 더는 안 닿는다. (mocking.server AC-6/AC-8) → `e2e/suites/21-api-mocking.mjs`
