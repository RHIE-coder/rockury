...
- **CASE-apistudio-003b (enum 편집 화면)** `string → enum` 전환이 열려 있다 — 빈 허용 목록은 저장이 거부되므로 **타입과 허용 값을 함께 커밋**한다. 값을 치기 전에는 경고만 뜨고 저장 오류 배너가 번쩍이지 않는다. (requests.template AC-7) → `e2e/suites/37-api-gaps.mjs`
...
- **CASE-apistudio-032** markdown 렌더: 링크·코드블록·표가 미리보기에서 올바르게 렌더된다. 코드블록 **안쪽은 안 꾸민다**(예제가 망가지면 안 된다). **`javascript:`·`data:` 는 링크로 안 만들고 원문 그대로 글자로 남긴다.** 지원하지 않는 문법도 추측해서 꾸미지 않는다. 어디까지 그리는지 화면이 말한다. (docs.authored AC-2) → `shared/api/markdown.test.ts` · `e2e/suites/37-api-gaps.mjs`
...
- **CASE-apistudio-051** 드롭 방지: 폴더를 자기 자손으로 옮기려 하면 거부하고 **이유를 준다**(조용히 무시하면 "왜 안 되지"가 된다). 경로 다듬기·조상 판정이 경계에서 안 헷갈린다(`a` 는 `a/b` 의 조상이지만 `ab` 의 조상은 아니다). 폴더를 옮기거나 이름을 바꾸면 자손 경로가 함께 따라간다. 검색 중에는 트리가 평평해진다. 요청을 폴더로 끌어다 놓으면 소속이 바뀐다. (tree AC-1) → `shared/api/tree.test.ts` · `e2e/suites/37-api-gaps.mjs`
- **CASE-apistudio-051b (폴더 손잡이)** 폴더 행에서 **이름을 바꾸고** 폴더째 **끌어 옮긴다**. 이름을 바꾸면 자손 경로가 함께 따라간다. **이름이 겹치면 막는다** — 안 막으면 두 폴더가 조용히 하나로 합쳐지고, 어느 요청이 원래 어느 쪽이었는지는 되돌릴 근거가 없다. 자기 자손 안으로도 못 옮긴다(트리가 고리가 된다). 막힌 이유는 트리 위에 글자로 뜬다 — 조용히 아무 일도 안 일어나면 "왜 안 되지"가 된다. (requests.tree) → `shared/api/tree.test.ts` · `e2e/suites/37-api-gaps.mjs`
- **CASE-apistudio-052** 상태 표식 판정: 판정 기록이 없는 요청은 **미관측**이고 일치가 아니다. 일치/어긋남/미관측 세 상태가 섞이지 않는다. (tree AC-4 · `api-service.md` §4-①)

## Scenario S7 — 앱 구동 흐름 (e2e/suites/30-api-studio, CSS/text 로케이터만)
...
- **CASE-apistudio-062** 본문에 `{{함수()}}` 입력 → **편집 중에** 치환 미리보기가 보인다. 비밀은 편집 화면에서도 가려진다. (template AC-6) → `e2e/suites/37-api-gaps.mjs`
...
## Scenario S8 — 모의 서버 (순수 로직 + 앱 구동)
- **CASE-apistudio-090** 가짜 본문 만들기: 선언한 타입대로 만들고 **값이 일부러 가짜처럼 보인다**(`(mock:이름)`). 허용 값이 있으면 그 값을 쓴다. 중첩 객체·배열 안까지 만들되 **원소 모양을 모르는 배열은 빈 배열**이다(원소를 지어내지 않는다). (mocking.server AC-3) → `shared/api/mock.test.ts`
- **CASE-apistudio-091** 필수여부 번역: `필수`는 늘, **`없을 수 있음`은 `null`** 로(프론트가 null 경로를 짜게), `모름`은 내되 **짐작 건수**를 함께 낸다. 짐작이 없으면 0 이다(없는 것과 0 은 다르다). (mocking.server AC-4)
- **CASE-apistudio-092 (지어내지 않는다)** 응답 모양 선언이 없으면 **가짜 본문을 안 만들고** `501` 과 사유를 준다 — 그럴듯한 JSON 을 주면 없는 계약 위에 화면을 짓게 된다. 맞는 선언이 아예 없는 경로는 `404` 이고 "흉내 낼 근거가 없다"고 말한다. 선언 안 한 상태를 골라도 지어내지 않는다. (mocking.server AC-2)
- **CASE-apistudio-093** 길 찾기: 메서드와 경로가 함께 맞아야 한다. `{{치환}}` 자리는 아무 조각이나 받고, 조각 수가 다르면 안 맞는다. 쿼리는 경로 대조에서 뺀다. (mocking.server AC-1)
- **CASE-apistudio-094** 상태 고르기: 기본은 **가장 낮은 2xx**, 없으면 첫 선언. 골라 둔 상태가 이기고 **서버를 안 끊는다**. (mocking.server AC-5)
- **CASE-apistudio-095 (강등 금지)** REST 단발만 흉내 낸다. 그 밖의 종류는 **흉내 내는 척하지 않고** 사유를 주며, 화면과 **창구 양쪽에서** 막는다. (mocking.server AC-7)
- **CASE-apistudio-096 (관측이 아니다)** 가짜 응답은 **실행 기록으로 안 남는다** — 남으면 판정이 "선언 대 선언"이 되어 늘 이상 없음이 된다. 응답에 가짜임을 밝히는 헤더가 붙는다. 켜면 로컬 주소가 뜨고 끄면 그 주소가 더는 안 닿는다. (mocking.server AC-6/AC-8) → `e2e/suites/38-api-mocking.mjs`

## 미구현 · 미검증 (조용한 통과 금지)
> 아래는 **정의는 있으나 아직 만들지 않은 것**이다. 케이스가 적혀 있다고 통과한 것이 아니다.
(폴더 이름 바꾸기·폴더째 끌어 옮기기는 2026-07-29 화면에 붙였다 —
 `CASE-apistudio-051b` · `e2e/suites/37-api-gaps.mjs`.)
