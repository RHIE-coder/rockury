...
### Surface db-console.diagram — ERD 캔버스 공통(배치·그룹·상세보기) (신설)
> 세 ERD 캔버스가 **같은 시각 레이어**를 쓴다: Studio › Diagram(가상 ERD) · Console › Diagram(읽기) · Console › Diagram(편집).
> 아래 배치·그룹·상세보기 규칙은 **셋 모두의 정본**이다(`db-studio.diagram` 은 이 절을 가리킨다).
> 다른 것은 저장 스코프뿐: **Console 은 연결(Connection)별, Studio 는 설계(Design)별**.

#### Section db-console.diagram.layout — 배치 영속
- **AC-1** 노드 위치와 화면 위치(뷰포트)를 스코프별로 영속한다. 스코프 키는 Console = 연결 id, Studio = `design:<설계 id>`. 노드 키는 이름 기반 결정적 id(`t:<테이블명>`)라 재역설계 뒤에도 짝이 맞는다.
...
#### Section db-console.diagram.group — 그룹(레이어)
...
#### Section db-console.diagram.group-panel — 왼쪽 패널 그룹 UI
...
- **AC-2b** **소속 테이블까지 지우는 선택지는 설계부(Studio)에만 있다.** 운영부(Console)의 확인 창에는 `그룹만 지우기` 뿐이다 — 거기 테이블은 실 DB 객체라, 지우는 길은 편집 모드의 대기 변경 → DDL 미리보기 → 트랜잭션 게이트 하나여야 한다(그룹 패널이 우회로가 되면 안 된다).
...
- **AC-3** 그룹의 테이블 항목을 누르면 캔버스가 그 노드로 이동한다(`db-console.diagram.table-panel` AC-2 와 같은 이동).
- **AC-4** **그룹만 보기** — 켠 그룹의 소속 테이블만 캔버스에 남긴다. `관계만` 필터와 함께 걸린다(둘 다 켜면 둘 다 적용).
- **AC-5** 접기 토글은 패널에도 있고, 캔버스의 접힘과 **같은 상태**다.

#### Section db-console.diagram.detail — 상세보기 서랍
- **AC-1** 캔버스에서 테이블을 고르면 **캔버스 아래 서랍**에 그 테이블 상세가 뜬다. 접고 펼 수 있고, 접으면 캔버스가 온전히 돌아온다. 아무것도 안 고른 상태의 기본은 닫힘이다.
- **AC-2** 서랍 내용은 **Definition 화면을 그대로 쓴다 — 따로 만들지 않는다.** 읽기(Console)는 `TableDetail`, 라이브 편집(Console › Diagram 편집)은 `EditableTableDetail`, 설계(Studio)는 `TableForm`. Definition 에서 되는 편집은 여기서도 그대로 된다(컬럼 추가·수정·삭제, 키 토글, 제약 편집 — `db-console.definition.detail` AC-1~4 가 그대로 적용된다).
- **AC-3** `[Table|SQL]` 토글로 DDL 원문을 보이고 복사한다 — Definition SQL 뷰와 **같은 컴포넌트·같은 방언 규칙**(`db-console.definition.sql`).
