...
## 구조 (스위트)
```
smoke.mjs             러너 — SUITES 를 순서대로 실행, 체크포인트 기록, 요약 출력
...
```
- 실행 순서 = **상태 의존 순서**(앞 스위트가 만든 설계·연결을 뒤가 쓴다) → 임의로 바꾸지 말 것.
...
새 영역이면 `suites/NN-이름.mjs` 를 만들고(`meta` + `run(ctx)`) 러너의 `SUITES` 에 등록한다.
