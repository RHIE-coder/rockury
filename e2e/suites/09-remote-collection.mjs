// 스모크 스위트 — Remote › Collection/History — 컬렉션 트리 DnD·Run All·실행 이력
// 실행: `npm run e2e`(e2e/smoke.mjs 러너가 순서대로 부른다). 단독 실행용 진입점은 없다.
// ⚠ 접근성 쿼리(getByRole 등)는 창을 크래시시킨다 → CSS/text 로케이터만.

export const meta = {
  name: '09-remote-collection',
  needsDb: true,
  desc: 'Remote › Collection/History — 컬렉션 트리 DnD·Run All·실행 이력'
}

export async function run(ctx) {
  const { check, click, body } = ctx
  let page = ctx.page
  // Remote › Collection — 좌 컬렉션 트리 · 중앙 아이템 · 우 QUERIES(재설계)
  await click('button:has-text("Collection")')
  await page.waitForTimeout(400)
  await click('button[title="새 컬렉션"]')
  await page.waitForTimeout(500)
  check('Remote › Collection: 컬렉션 생성', (await body()).includes('Untitled Collection'))
  ...
  await page.waitForTimeout(500)
  check('Remote › Collection: QUERIES 드래그 → 참조 추가(참조 배지)', (await body()).includes('참조'))
  ...
  check('Remote › Collection: 조회 전용 Run-All 은 커밋 불필요(자동 종료)', (await body()).includes('커밋 불필요') && !(await body()).includes('아직 커밋되지'))
  ...
  check('Remote › Collection: 쓰기 포함 Run-All 은 커밋 게이트', (await body()).includes('아직 커밋되지'))
  await click('button:has-text("롤백")')
  await page.waitForTimeout(300)
  check('Remote › Collection: 롤백 후 게이트 해제', !(await body()).includes('아직 커밋되지'))

  // 쓰기 아이템 개별 실행 — 커밋되지 않고 트랜잭션에 쌓임(원자성 유지) → 커밋
  await page.locator('button[title^="이 아이템만 실행"]').last().click()
  await page.waitForSelector('text=아직 커밋되지', { timeout: 15_000 })
  check('Remote › Collection: 쓰기 아이템 개별 실행 → 미커밋(원자성)', (await body()).includes('아직 커밋되지'))
  await click('button:has-text("커밋")')
  await page.waitForTimeout(300)
  check('Remote › Collection: 개별 실행 커밋 후 게이트 해제', !(await body()).includes('아직 커밋되지'))
  ...
  check('Remote › Collection: 트리 DnD 수직 드래그=루트 유지(folderId null)', (await folderOf(treeIds.moveId)) == null)
  await remountColl()
  await dragTree('TREE_MOVE', 'TREE_CHILD', 22)
  check('Remote › Collection: 트리 DnD 오른쪽 드래그=폴더로 중첩(양성 대조)', (await folderOf(treeIds.moveId)) === treeIds.faId)
  ...
  check('Remote › Collection: 폴더 아이콘 클릭 → 접힘(자식 숨김)', !(await colHasRow('TREE_CHILD')))
  await iconClick('TREE_FOLDER')
  check('Remote › Collection: 폴더 아이콘 클릭 → 펼침(자식 복귀)', await colHasRow('TREE_CHILD'))
  ...
  check('Remote › Collection: 드롭 전 접힌 상태 확인', !(await colHasRow('AXCHILD')))
  ...
  check('Remote › Collection: 접힌 폴더로 드롭 시 자동 펼침(아이템 안 사라짐)', (await colHasRow('AXCHILD')) && (await folderOf(ax.dropId)) === ax.fId)
  ...
  check('Remote › Collection: 우클릭 컨텍스트 메뉴 등장', await page.locator('button:has-text("이동")').count() > 0)
  // ⭐ 바깥 클릭/Esc 로 닫힌다 (사용자 회귀: 다른 화면 눌러도 안 닫힘)
  await page.keyboard.press('Escape')
  await page.waitForTimeout(200)
  check('Remote › Collection: 컨텍스트 메뉴 Esc 로 닫힘', await page.locator('button:has-text("이동")').count() === 0)
  await page.locator('div.group\\/row:has-text("CTXMOVE")').first().click({ button: 'right' })
  await page.waitForTimeout(250)
  ...
  await page.waitForTimeout(300)
  check('Remote › History: 실행 이력 기록(Query SQL)', (await body()).includes('SELECT id, email FROM users'))
  ...
  check('Remote › History: 같은 SQL 3번 실행 = 3행(중복 접기 없음)', histCount === 3)
  ...
  await page.waitForSelector('text=Source', { timeout: 8_000 })
  await page.waitForTimeout(300)
  check('Remote › History: 컬렉션 실행 그룹(GRP_COLL · 2개 쿼리)', (await body()).includes('GRP_COLL') && (await body()).includes('2개 쿼리'))
