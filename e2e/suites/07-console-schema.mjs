...
export async function run(ctx) {
  ...
  // ⭐ CASE-console-04C — 화면 이동(줌) **직후** 다른 화면으로 떠나도 저장된다.
    ...
    })
  {
    ...
    check('Console › Diagram: 화면 이동 직후 떠나도 저장됨(지연 저장 마무리)', !!after && after !== before)
    await click('button:has-text("Diagram")')
    await page.waitForSelector('.react-flow__node[data-id="t:users"]', { timeout: 15_000 })
    await page.waitForTimeout(400)
    check('Console › Diagram: 왕복 후 화면 위치도 복원', (await savedViewport()) === after)
  }

  // ⭐ 그룹(레이어) — 정본 §db-console.diagram.group / .group-panel
...

  {
    ...
    check('Console › Diagram: 좌측 `그룹` 탭 존재', (await page.locator('[data-diagram-group-panel]').count()) > 0)

    await page.locator('[data-group-create]').first().click()
    await page.keyboard.press('Escape') // 이름 바꾸기 입력 닫기(기본 이름 유지)
    await page.waitForTimeout(250)
    check('Console › Diagram: 그룹 만들기 → 목록에 뜬다', (await page.locator('[data-group-row="그룹 1"]').count()) > 0)
    ...
    check(
      'Console › Diagram: 패널에서 그룹에 넣기 → 캔버스에 그룹 영역이 그려진다',
    ...
    check(
      'Console › Diagram: 소속이 저장본에 2개로 들어간다',
    ...
    // AC-1 — 그룹 상자는 테이블·관계선보다 **뒤에** 깔린다(내용을 가리지 않는다).
    {
      ...
      check('Console › Diagram: 그룹 상자가 테이블·관계선보다 뒤에 깔린다', Number(z) < 0)
    }
    ...
    await fitView()
    {
      const grab = await ctx.nodeGrabPoint(page.locator('.react-flow__node[data-id="t:users"]').first())
      check('Console › Diagram: 그룹 안 테이블을 집을 수 있음', !!grab)
      if (grab) {
        const before = await nodeXY('t:user_roles')
        await page.mouse.move(grab.from.x, grab.from.y)
        await page.mouse.down()
        await page.mouse.move(grab.from.x - 50, grab.from.y - 40, { steps: 8 })
        await page.mouse.up()
        await page.waitForTimeout(400)
        const after = await nodeXY('t:user_roles')
        check(
          'Console › Diagram: 그룹 안 테이블 하나만 끌면 그것만 움직인다',
      ...
      check(
        'Console › Diagram: 개별 이동 뒤 소속 2개 전제 회복',
        (await savedGroups()).some((g) => g.id === 'g1' && g.n === 2)
      )
    }
    ...
    //   눌리는 게 운에 달린다(실측 flake). `그룹만 보기` 로 그 그룹만 남기면 배율이 크게 오른다.
    {
      ...
      // AC-6a — 상자 크기 손조절 → 저장 → `자동 크기` 로 되돌리기.
      {
        const h = await page
          ...
          .first()
          .boundingBox()
        check('Console › Diagram: 그룹 크기 조절 손잡이 존재', !!h)
        if (h) {
          ...
          check('Console › Diagram: 손으로 조절한 크기가 저장된다', !!sized && sized.w > 0 && sized.h > 0)
          await page.locator('[data-group-autosize="그룹 1"]').first().click()
          await page.waitForTimeout(600)
              check('Console › Diagram: `자동 크기` 로 손 크기가 지워진다', (await savedGroup())?.w == null)
          ...
          check('Console › Diagram: 그룹 상자 가장자리가 클릭을 가로채지 않는다', edgeEatsClick === 'ok')
        }
      }

      // CASE-console-04E — 캔버스 이름표의 접기 버튼으로 접고 펴기.
      {
        const pt = await clickable('[data-erd-group-toggle="그룹 1"]')
        check('Console › Diagram: 캔버스 그룹 접기 버튼을 누를 수 있음', !!pt)
        if (pt) {
          const posBefore = await nodeXY('t:users')
          await page.mouse.click(pt.x, pt.y)
          await page.waitForTimeout(600)
          check(
            'Console › Diagram: 그룹 접기 → 소속 노드 숨고 상자만 남는다',
            (await page.locator('.react-flow__node[data-id="t:users"]').count()) === 0 &&
          ...
          await page.waitForTimeout(700)
          check(
            'Console › Diagram: 그룹 펴기 → 소속 노드 복귀',
            (await page.locator('.react-flow__node[data-id="t:users"]').count()) === 1
          )
          // AC-5a 회귀 — 펴면 **접기 전 자리**로 돌아와야 한다(자동 배치 자리로 튀면 배치가 사라진다).
          const posAfter = await nodeXY('t:users')
          check(
            'Console › Diagram: 펴면 접기 전 자리로 돌아온다',
            !!posBefore && !!posAfter && near(posBefore.x, posAfter.x) && near(posBefore.y, posAfter.y)
          )
        }
      }

      // CASE-console-04I — 영역 밖으로 끌어내면 소속이 풀린다(멤버십을 바꾸므로 이 구간 마지막에).
      //   ⚠ 반대 방향(안으로 끌어 넣기)은 앱 흐름으로 안 덮는다 — 사유는 CASE-console-04I 주석 참고.
      {
        const out = await boxOf()
        const grabOut = await ctx.nodeGrabPoint(page.locator('.react-flow__node[data-id="t:users"]').first())
        check('Console › Diagram: 끌어내기 검사용 노드·상자 확보', !!grabOut && !!out)
        if (grabOut && out) {
          const before = await savedPos('t:users')
          ...
          const moved = await savedPos('t:users')
          check('Console › Diagram: 끌어내기 드래그가 실제로 일어남', !!before && !!moved && moved.x > before.x + 20)
          check('Console › Diagram: 영역 밖으로 끌어내면 그룹에서 빠진다', (await savedGroup())?.n === 1)
        }
      }
      ...
      check('Console › Diagram: 패널로 다시 넣으면 소속 복귀', (await savedGroup())?.n === 2)
    }

    // CASE-console-04D — 이름표를 끌면 소속 테이블이 같은 거리만큼 함께 움직인다.
    await fitView()
    {
      const u0 = await nodeXY('t:users')
      const r0 = await nodeXY('t:user_roles')
      const pt = await clickable('[data-erd-group="그룹 1"] .erd-group-handle')
      check('Console › Diagram: 그룹 이름표가 오버레이에 안 덮임', !!pt)
      if (pt) {
        ...
        const u1 = await nodeXY('t:users')
        const r1 = await nodeXY('t:user_roles')
        check(
          'Console › Diagram: 그룹 이름표를 끌면 소속 테이블이 같이 움직인다',
          u1.x - u0.x > 30 && near(u1.x - u0.x, r1.x - r0.x) && near(u1.y - u0.y, r1.y - r0.y)
        )
      }
    }

    // CASE-console-04F — 그룹만 보기 / 그룹 삭제는 테이블을 지우지 않는다.
    ...
    check('Console › Diagram: 그룹만 보기 → 그 그룹 소속만 남는다', onlyCount === 2 && onlyCount < allTables)
    await page.locator('[data-group-only="그룹 1"]').first().click()
    await page.waitForTimeout(450)
    check('Console › Diagram: 그룹만 보기 끄면 전체 복귀', (await tableNodes()) === allTables)

    // 지우기는 **버리는 그룹**으로 확인한다 — `그룹 1` 은 콜드 재시작 영속 검사(12)까지 남긴다.
    await page.locator('[data-group-create]').first().click()
    await page.keyboard.press('Escape')
    await page.waitForTimeout(250)
    check('Console › Diagram: 버리는 그룹(그룹 2) 생성', (await page.locator('[data-group-row="그룹 2"]').count()) === 1)
    ...
    check(
      'Console › Diagram: 자동 배치 후에도 그룹·소속이 남는다',
    ...
    await page.waitForTimeout(400)
    check(
      'Console › Diagram: 지우기 → 확인 창이 뜨고 아직 안 지워진다',
      (await page.locator('[data-group-delete-dialog="그룹 1"]').count()) === 1 &&
        (await page.locator('[data-group-row="그룹 1"]').count()) === 1
    )
    check(
      'Console › Diagram: 운영부 확인 창에는 테이블 동반 삭제가 없다',
      (await page.locator('[data-group-delete-with-tables]').count()) === 0
    )
    await page.keyboard.press('Escape')
    await page.waitForTimeout(400)
    check('Console › Diagram: 취소하면 그룹이 그대로', (await page.locator('[data-group-row="그룹 1"]').count()) === 1)
    ...
    await page.waitForTimeout(500)
    check(
      'Console › Diagram: 그룹만 지우기 → 그룹은 사라지고 테이블은 남는다',
  ...
  // ⭐ CASE-console-04G — 상세보기 서랍(내용은 Definition 화면 그대로) + SQL + FK 이동 + 크게 보기.
  {
    await page.locator('.react-flow__node[data-id="t:user_roles"]').first().click()
    await page.waitForTimeout(400)
    check('Console › Diagram: 노드 클릭 → 아래 상세 서랍이 열린다', (await page.locator('[data-diagram-drawer="open"]').count()) > 0)
    const drawer = page.locator('[data-diagram-drawer="open"]')
    const drawerText = await drawer.innerText()
    check(
      'Console › Diagram: 서랍에 컬럼·제약이 뜬다(Definition 화면 그대로)',
      /constraints/i.test(drawerText) && drawerText.includes('user_id')
    )

    await page.locator('[data-drawer-form="sql"]').first().click()
    await page.waitForTimeout(300)
    check('Console › Diagram: 서랍 SQL 토글 → CREATE 문', (await drawer.innerText()).includes('CREATE'))
    ...
    await page.waitForTimeout(900)
    check(
      'Console › Diagram: 상세의 FK 참조를 누르면 그 테이블로 이동하고 서랍도 바뀐다',
      (await page.locator('[data-diagram-drawer="open"]').innerText()).includes('users')
    )

    await page.locator('[data-drawer-expand]').first().click()
    await page.waitForTimeout(400)
    check('Console › Diagram: 크게 보기 → 모달', (await page.locator('[data-drawer-modal]').count()) > 0)
    await page.keyboard.press('Escape')
    await page.waitForTimeout(300)
    check('Console › Diagram: 모달 닫힘', (await page.locator('[data-drawer-modal]').count()) === 0)
  ...
  check('Console › Diagram 편집: 누를 수 있는 테이블 노드 확보', !!editTarget)
  if (editTarget) await page.mouse.click(editTarget.from.x, editTarget.from.y)
  await page.waitForTimeout(500)
  check(
    'Console › Diagram 편집: 노드 선택 → 상세 서랍이 Definition 편집 화면을 그대로 연다',
  ...
  await page.waitForTimeout(400)
  check(
    'Console › Diagram 편집: 캔버스 + → 노드 증가 + 대기 변경',
