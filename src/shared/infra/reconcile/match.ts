import type { LiveResource, MatchBasis, ReconNode } from '../types'

/**
 * 설계 노드 ↔ 실물 짝짓기. **메인·렌더러가 같은 규칙을 본다**(공용으로 올린 근거는 `../types.ts`).
 *
 * 노드 타입을 열어 둔(제네릭) 이유: 렌더러는 좌표·문서까지 달린 `DesignNode` 를 넘기고 메인은
 * SQLite 행을 넘긴다. 결과에 **넘긴 그 객체가 그대로 실려 나가야** 렌더러가 문서 유무를
 * 이어서 볼 수 있다 — 최소 모양으로 깎아 돌려주면 호출한 쪽이 다시 찾아 붙여야 한다.
 */
...
export interface MatchResult<N extends ReconNode = ReconNode> {
  matches: Match[]
  /** 짝 없는 설계 노드 — '미구축' 후보. */
  unmatchedDesign: N[]
  /** 짝 없는 실물 — '미등록' 후보. */
 ...
 */
export function matchResources<N extends ReconNode>(
  nodes: N[],
  resources: LiveResource[]
): MatchResult<N> {
