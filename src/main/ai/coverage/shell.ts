...
export const shellCoverage: ServiceCoverage = {
  ...
  excluded: {
    ...
    'window:strip':
      ...
      '에이전트가 읽거나 쓸 것이 없다.',
    'window:tearOff':
      'UI 창 제어 — 사람이 탭을 손으로 빼내는 동작의 한 도막이다. window:open 과 같은 이유로 ' +
      '닫는다(에이전트가 부르면 사용자 화면에 창이 제멋대로 뜬다).',
    'window:dragSelf':
      'UI 창 제어 — 마지막 한 장을 끌 때 창 자신을 끌기 대상으로 세우는 도막이다. ' +
      '사람 손동작의 한 조각이라 단독으로 부를 것이 없다.',
    'window:dragMove': 'UI 창 제어 — 끌고 있는 창을 커서 따라 옮기는 도막. 단독으로 부를 것이 없다.',
    'window:dragEnd': 'UI 창 제어 — 끌기를 끝맺는 도막. 단독으로 부를 것이 없다.',
