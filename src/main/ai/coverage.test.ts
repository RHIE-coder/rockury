...
import { AI_EXCLUDED_CHANNELS, AI_TOOL_CHANNELS, SERVICE_COVERAGE } from './coverage'
...
describe('MCP 커버리지 핀', () => {
  ...
  const exposed = new Set(Object.values(AI_TOOL_CHANNELS).flat())
  const excluded = new Set(Object.keys(AI_EXCLUDED_CHANNELS))
  ...
  it('모든 IPC 채널은 MCP 노출 또는 제외(사유 포함)로 등재된다 — 새 채널을 만들었으면 coverage.ts 를 갱신하라', () => {
    ...
    expect(missing, `MCP 노출/제외 미등재 채널: ${missing.join(', ')} → src/main/ai/coverage/<서비스>.ts 에 도구 대응 또는 제외 사유를 등재하세요`).toEqual([])
  })
  ...
  it('지도의 도구 키 = 실제 등록된 MCP 도구(TOOL_NAMES)', () => {
    expect(new Set(Object.keys(AI_TOOL_CHANNELS))).toEqual(new Set(TOOL_NAMES))
  })

  it('제외 사유는 비어 있지 않다 — "왜 노출 안 하는지"를 반드시 적는다', () => {
    for (const [ch, reason] of Object.entries(AI_EXCLUDED_CHANNELS)) {
      expect(reason.trim().length, `${ch} 제외 사유 비었음`).toBeGreaterThan(4)
    }
  })

  // CASE-ai-063 — 삭제류 미노출 핀: "파괴적 조작은 사람이 앱에서만"(spec tools.write AC-7)을
  ...
  it('삭제류는 노출되지 않는다 — 파괴 채널은 확정 사유와 함께 제외 지도에만', () => {
    ...
    for (const ch of ['designs:delete', 'versions:delete']) {
      expect(AI_EXCLUDED_CHANNELS[ch], `${ch} 는 확정 사유로 남아야 한다`).toMatch(/사람이 앱에서만/)
    }
...
describe('MCP 커버리지 핀 — 서비스별 분할 이후', () => {
  it('CASE-pdev-010 하위 폴더(ipc/<서비스>/)의 채널까지 전수 수집한다', () => {
    ...
    for (const ch of ['designs:list', 'query:run', 'col:list', 'ai:status', 'window:close']) {
  ...
  it('CASE-pdev-014 안전핀: 합쳐진 제외 지도가 비면 실패 (등록부가 통째로 날아간 상태 감지)', () => {
    expect(Object.keys(AI_EXCLUDED_CHANNELS).length).toBeGreaterThan(10)
  })
})
