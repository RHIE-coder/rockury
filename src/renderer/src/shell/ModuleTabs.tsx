...
import { groupBySlot, isAreaSplit } from '../nav/moduleSlots'
  ...
  return (
    <Tabs.Root value={module.id} onValueChange={selectModule} className="shrink-0">
      ...
      >
         ...
         * 지키고 선이 먼저 줄어든다.
         */}
        <Zone modules={zones.start} split={split} className="shrink-0 justify-start" />
        {crossing ? (
          <>
            <Bridge side="trailing" />
            <Zone modules={zones.center} split={split} className="shrink-0" />
            <Bridge side="leading" />
          </>
        ) : (
          <span aria-hidden className="flex-1" />
        )}
        <Zone modules={zones.end} split={split} className="shrink-0 justify-end" />
      </Tabs.List>
    </Tabs.Root>
  )
}
...
/** 줄의 한 자리(`Module.slot`). 구획이 시작할 때마다 구분선과 뱃지가 앞에 선다. */
function Zone({
  modules,
  split,
  className
