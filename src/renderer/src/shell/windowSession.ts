 ...
 * 두 창이 "마지막에 고른 설계·프로젝트 범위"를 서로 덮어써서, 앱을 껐다 켰을 때 어느 창의
 * 상태가 남을지가 마지막으로 움직인 창에 따라 달라진다.
 *
 * 그래서 **저장소의 주인은 첫 창 하나**다. 떼어낸 창은 읽기만 한다 — 읽기를 막지 않는 이유는
 * 첫 화면 때문이다. 떼어낸 창도 지금 고른 대상을 물려받고 시작해야 "이 화면을 하나 더"가 된다.
 * 거기서 대상을 바꾸면 그 창 안에서만 갈린다(스토어가 창마다 따로 산다).
 *
 * **탭 목록은 여기 안 든다 — 메인 프로세스가 주인이다**(`main/windows.ts`). 창 밖에 두어야
 * 창끼리 안 부딪히고, 껐다 켤 때 창 배치까지 되살아난다.
 */
