...
import { ErdCanvas, ToggleChip } from './ErdCanvas'
...
export interface DiagramSurfaceProps {
   ...
   */
  detail: (table: TableDef, jump: (tableName: string) => void) => ReactNode
 ...
 * Studio › Diagram · Console › Diagram(읽기/편집) 셋이 이걸 그대로 쓴다.
 * ⚠ `ReactFlowProvider` **안쪽**에 놓아야 한다(패널·캔버스가 같은 flow 를 본다).
 * 정본: `docs/spec/db-console.md` §db-console.diagram.
 */
export function DiagramSurface({
  ...
  /** 테이블을 고르면 서랍을 연다 — 눌렀는데 아무 일도 안 일어나면 고장으로 보인다. */
