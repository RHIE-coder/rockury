...
const nodeTypes = { tableErd: TableErdNode, erdGroup: GroupErdNode }
...
/**
 * ERD 캔버스(공용) — Studio › Diagram(가상) · Console › Diagram(읽기) · Console › Diagram(편집)
 * 셋이 **이 한 벌**을 쓴다. 정본: `docs/spec/db-console.md` §db-console.diagram.
...
export function ErdCanvas({
  ...
  const base = useMemo(() => toFlow(shown, editable, clusters), [shown, editable, clusters])
  ...
  const persistNow = useCallback(() => {
    if (!persist) return
    const merged = mergePositions(layoutRef.current, positionsOf(rf.getNodes().filter((n) => n.type === 'tableErd')), allTableIds)
