...
/**
 * Design ERD 편집: 관계선을 끌었을 때(source 컬럼 → target 테이블) 만들 FK 제약 패치를 계산하는
 ...
 * - refTable/refSchema: 대상 테이블(스키마까지 — 같은 이름이 여러 스키마에 있을 수 있다)
 * - refColumns: 대상 PK(기본)
...
export function buildFkPatch(
  ...
  target: TableDef
): Pick<Constraint, 'columns' | 'refTable' | 'refSchema' | 'refColumns'> | null {
  const col = source.columns.find((c) => c.id === sourceColumnId)
  if (!col) return null
  return {
    columns: [{ columnId: sourceColumnId }],
    refTable: target.name,
    // 같은 스키마면 비워 두지 않고 채운다 — 나중에 테이블이 다른 스키마로 옮겨가도 참조가 안 흔들린다.
