...
// 이름으로만 대상을 찾던 시절엔 선이 **엉뚱한 스키마의 동명 테이블**로 갔다 —
// 오류가 안 나고 그림만 틀리는 종류라 눈으로는 못 잡는다.
describe('buildErd — 여러 스키마', () => {
  const scoped = (schema: string, name: string, columns: Column[], constraints: Constraint[] = []): TableDef => ({
    ...table(name, columns, constraints),
    id: `t:${schema}.${name}`,
    schema
  })

  it('교차 스키마 FK 는 refSchema 가 가리키는 테이블로 이어진다', () => {
    const { edges } = buildErd([
      scoped('public', 'users', [col('users', 'id')], [pk('users', ['id'])]),
      scoped('auth', 'users', [col('users', 'id')], [pk('users', ['id'])]),
      scoped(
        'public',
        'posts',
        [col('posts', 'id'), col('posts', 'author_id')],
        [fk('posts', 'fk_author', ['author_id'], 'users', ['id'], { refSchema: 'auth' })]
      )
    ])
    expect(edges).toHaveLength(1)
    expect(edges[0].target).toBe('t:auth.users')
  })

  it('refSchema 가 없으면 같은 스키마 안에서 찾는다', () => {
    const { edges } = buildErd([
      scoped('public', 'users', [col('users', 'id')], [pk('users', ['id'])]),
      scoped('auth', 'users', [col('users', 'id')], [pk('users', ['id'])]),
      scoped(
        'auth',
        'sessions',
        [col('sessions', 'id'), col('sessions', 'user_id')],
        [fk('sessions', 'fk_user', ['user_id'], 'users', ['id'])]
      )
    ])
    expect(edges[0].target).toBe('t:auth.users')
  })

  it('범위 밖(안 켠 스키마)을 가리키는 FK 는 선을 안 만든다 — 없는 관계를 그리지 않는다', () => {
    const { edges } = buildErd([
      scoped(
        'public',
        'posts',
  ...
  it('자기참조는 이름이 아니라 대상 노드가 자기 자신인지로 가른다', () => {
    const { edges } = buildErd([
      scoped('public', 'users', [col('users', 'id')], [pk('users', ['id'])]),
      scoped(
        'auth',
        'users',
        [col('users', 'id'), col('users', 'parent_id')],
        [pk('users', ['id']), fk('users', 'fk_parent', ['parent_id'], 'users', ['id'])]
      )
    ])
    // auth.users → auth.users 는 자기참조, public.users 를 가리킨 것이 아니다.
    expect(edges.map((e) => [e.target, e.selfRef])).toEqual([['t:auth.users', true]])
  })
})

// 범위 밖 참조(§db-remote.scope R3) — 아무것도 안 그리면 관계가 있다는 사실 자체가 사라진다.
describe('buildErd — 범위 밖 대상 카드', () => {
  const scoped = (schema: string, name: string, columns: Column[], constraints: Constraint[] = []): TableDef => ({
    ...table(name, columns, constraints),
    id: `t:${schema}.${name}`,
    schema
  ...
  const posts = () =>
    scoped(
      'public',
      'posts',
      [col('posts', 'id'), col('posts', 'plan_id')],
      [fk('posts', 'fk_plan', ['plan_id'], 'plans', ['id'], { refSchema: 'billing' })]
    )

  it('스키마 목록을 주면 밖 대상을 카드로 만들고 선을 잇는다', () => {
    const { outsideNodes, edges } = buildErd([posts()], ['public', 'billing'])
    expect(outsideNodes).toEqual([
      { id: 'out:billing.plans', target: { schema: 'billing', name: 'plans' }, kind: 'addable' }
    ])
    expect(edges).toHaveLength(1)
    expect(edges[0].target).toBe('out:billing.plans')
  })

  it('켤 수 없는 스키마면 카드는 그리되 종류가 다르다 — 대처가 다르기 때문', () => {
    expect(buildErd([posts()], ['public']).outsideNodes[0].kind).toBe('unavailable')
  })

  it('스키마 목록을 안 주면 예전 그대로 — 밖 참조는 카드도 선도 없다', () => {
    const { outsideNodes, edges } = buildErd([posts()])
    expect(outsideNodes).toEqual([])
    expect(edges).toEqual([])
  })

  it('범위 안에서 이어지는 관계는 밖 카드를 안 만든다', () => {
    const { outsideNodes, edges } = buildErd(
      [posts(), scoped('billing', 'plans', [col('plans', 'id')], [pk('plans', ['id'])])],
      ['public', 'billing']
    )
    expect(outsideNodes).toEqual([])
    expect(edges[0].target).toBe('t:billing.plans')
  })
})
