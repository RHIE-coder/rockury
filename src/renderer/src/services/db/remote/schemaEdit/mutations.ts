import { groupBySchema } from '../../schemaRef'
...
/**
 * Remote 라이브 스키마 편집의 **순수 reducer**(입력→출력 결정적 → 테스트 의무).
 * draft(편집본) `TableDef[]` 를 변형만 한다 — 저장·DDL 실행은 store 가 한다.
 *
 * Design 편집(설계 draft)과 도메인 규칙은 같지만, 여기선:
...
export function addTable(
  ...
  // 새 테이블은 **지금 보고 있는 스키마**에 만든다. 범위에 여럿이 켜져 있으면 첫 번째
  // (스키마 이름순 첫 묶음)를 쓴다 — 스키마 없이 만들면 어디에 생기는지가 연결 기본값에
  // 맡겨져, 화면에서 본 자리와 실제로 만들어진 자리가 어긋난다.
  const schema = groupBySchema(tables)[0]?.schema || undefined
