...
/**
 * Remote 라이브 스키마 편집 스토어(연결 단위) — Definition·Diagram 이 **공유**한다.
 ...
 * 적용 성공 시 재역설계(useRemoteStore.load force)하고 편집을 종료한다.
 *
...
export const useSchemaEditStore = create<SchemaEditState>()((set, get) => {
  ...
  return {
    ...
    apply: async (dialect) => {
      ...
      // 성공 → 재역설계 후 편집 종료.
      await useRemoteStore.getState().load(connId, connId, true).catch(() => {})
